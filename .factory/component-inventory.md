...
| `ActionComposer` | Creates/edits one request | draft, invalid, saving, saved, error | Labels/errors/summary; focus first invalid field | M1 |
...
| `ShareLinkPanel` | Issue/copy/expire/revoke link | none, active, copied, expired, revoked, error | Copy result live-announced; revoke has specific confirmation | M1 |
...
| `ChoiceField` | Captures one listed option | ready, invalid, submitting, complete | `fieldset`/radio semantics and clear validation | M3 |
| `UploadTray` | Selects files and shows scan outcome | empty, selected, uploading, scanning, accepted, rejected | Native file input remains available; per-file progress/status | M3 |
| `ReminderControl` | Schedules/sends transactional reminder | idle, scheduled, sending, sent, error | Cadence and recipient named; result announced | M4 |
...
- The component gallery added in M1 is development-only or protected from indexing. It covers every implemented state without becoming a shipped product route.
