  ...
  <main id="main" class="landing" tabindex="-1">
    <section class="hero" aria-labelledby="home-title">
      <div class="hero-copy">
        ...
        <ul class="fact-lines" aria-label="Product facts">
          <li>A client can approve a request without creating an account.</li>
    ...
    <section class="product-preview" aria-labelledby="preview-title">
      ...
      <div class="preview-ledger">
        ...
        <div><h3>Open requests are ordered by deadline</h3><p>The client sees only the action named by the link. An approval records the decision, actor label, and server time.</p></div>
        <a class="text-link" href="/demo" onclick={(event) => { event.preventDefault(); navigate('/demo'); }}>Open the working approval demo</a>
      </div>
    </section>
    ...
    <section id="how-it-works" class="steps" aria-labelledby="steps-title">
      ...
      <ol>
        ...
        <li><span>2</span><div><h3>Let the client answer</h3><p>The client approves or asks for a change. No client account is needed.</p></div></li>
    ...
    <section class="boundary" aria-labelledby="boundary-title">
      ...
      <div class="boundary-copy">
        <p>Client Action Room does not edit documents, run chat, or collect payments. It keeps a focused request and its answer.</p>
        <p>The sample uses a temporary server-side room. It does not send email, open billing, or write to a real firm account.</p>
...
{:else if route === 'demo'}
  <main id="main" class="app-page" tabindex="-1">
    <section class="page-intro">
      <p class="eyebrow">Northline Studio · sample workspace</p>
      <h1 tabindex="-1">Your sample client action room</h1>
      <p>Publish the menu approval, answer as Maya, then read the dated record.</p>
    </section>
    {#if notice}<p class="inline-notice success" role="status">{notice}</p>{/if}
    {#if error}
    ...
    {:else if demo}
      <div class="workspace-header">
        ...
        <dl><div><dt>Owner</dt><dd>{demo.staff_owner}</dd></div><div><dt>Client</dt><dd>{demo.client_actor}</dd></div><div><dt>Demo ends</dt><dd>{formatDate(demo.expires_at)}</dd></div></dl>
      </div>
      <div class="staff-layout">
            ...
            {#each sortedActions as action (action.id)}
              <li class="action-slip" data-due={action.due_at} data-kind={action.kind}>
                ...
                {#if action.preview_only}
                  <p class="preview-note">Preview only · this action type is not available yet.</p>
                {:else if action.status === 'completed'}
                  <p class="recorded-note">Maya’s answer is in the audit record.</p>
                {:else}
                  <button class="button primary compact" type="button" disabled={busy} onclick={() => publish(action)}>Publish client link</button>
                {/if}
              </li>
            {/each}
          </ol>
        ...
        <aside class="detail-sheet" aria-labelledby="share-title">
          ...
          <h2 id="share-title">Share the approval</h2>
          <p>The link can read and answer only the menu approval. It expires after seven days.</p>
          {#if publishedLink}
          ...
          {:else}
            <p class="empty-note">Publish the approval to create its client link.</p>
          {/if}
          ...
          <ol>
            {#each demo.audit as event (event.id)}
              <li data-event={event.event_name}>
                ...
                <div><strong>{formatEventName(event)}</strong><p>{event.actor_label}{event.decision ? ` · ${event.decision === 'approved' ? 'Approved' : 'Changes requested'}` : ''}</p></div>
  ...
  <main id="main" class="client-page" tabindex="-1">
    ...
    {:else if client}
      <section class="client-window">
        ...
        <dl class="scope-list"><div><dt>Deadline</dt><dd><time datetime={client.action.due_at}>{formatDate(client.action.due_at)}</time></dd></div><div><dt>This link can see</dt><dd>This approval only</dd></div><div><dt>Link expires</dt><dd><time datetime={client.link_expires_at}>{formatDate(client.link_expires_at)}</time></dd></div></dl>
        {#if notice}<p class="inline-notice success" role="status">{notice}</p>{/if}
        {#if submission}
...
{:else if route === 'privacy'}
  <main id="main" class="prose-page" tabindex="-1">
    ...
    <p class="lede">The current demo stores only temporary sample changes. It does not create a firm account.</p>
    ...
    <section><h2>What we do not collect</h2><p>The demo has no analytics, advertising, account profiles, payment collection, file uploads, or email delivery. It does not call a third-party service.</p></section>
